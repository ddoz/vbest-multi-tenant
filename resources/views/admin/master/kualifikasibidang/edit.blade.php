...
@section('content')
				<div class="col-sm-12">
					<div class="card card-wrapper">
                        ...
                        <form action="{{route('master-klasifikasi-bidang.update',$kualifikasi->id)}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="kualifikasi_id" value="$kualifikasi->id">
                            <div class="form-group row">
                                <label class="col-sm-4 col-form-label">Nama Kualifikasi</label>
                                <div class="col-sm-6">
                                    <input type="text" value="{{$kualifikasi->nama_kualifikasi}}" class="form-control @error('nama_kualifikasi') is-invalid @enderror" value="{{old('nama_kualifikasi')}}" placeholder="Nama Kualifikasi" name="nama_kualifikasi">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-6">
                                    <select name="parent_id" id="parent_id" class="form-control select2">
                                        ...
                                        @foreach($kualifikasiAll as $kb)
                                        <option @if($kb->id == $kualifikasi->parent_id) selected @endif value="{{$kb->id}}">{{$kb->nama_kualifikasi}}</option>
                                        @endforeach
                                    </select>
