    ...
    public function index(Request $request) {
        if ($request->ajax()) {
            $data = RekeningBank::join('banks','banks.id','=','rekening_banks.bank_id')
                                ->where('rekening_banks.user_id', Auth::user()->id)
                                ->select('rekening_banks.*','banks.nama_bank')
                                ->get();
            return DataTables::of($data)->addIndexColumn()
                ->addColumn('action', function($row){
                    $btn = '<div class="d-flex flex-row"><form onsubmit="return confirm(\'Hapus Data?\')" class="formDelete" action="'.route('rekening-bank.destroy',$row->id).'" method="POST">
                                <button type="submit" class="btn btn-danger btn-sm"><span class="ti ti-trash"></span></button>
                                <a class="btn btn-primary btn-sm" href="'.route('rekening-bank.edit',$row->id).'"><span class="ti ti-pencil"></span></a>
                                <a class="btn btn-success btn-sm" href="'.route('rekening-bank.show',$row->id).'"><span class="ti ti-search"></span></a>
                                '.csrf_field().'
        ...
        return view('rekening-bank/index');
    }
